				...
				if (d >= 6) {
					...
					for (int playPan = 0; playPan < 3; playPan++) {
						...
						if (playPan == 2 && saveScrollType < 2) {
							Texture2D.DrawStaticAsset(graphics, Texture2D.Get("TestUIArrow2"), k - 22 + savWidth,
									l - 32, null, 2f, 2f);
						}
						if (playPan == 0 && saveScrollType > 0) {
							Texture2D.DrawStaticAsset(graphics, Texture2D.Get("TestUIArrow"), k - 22 - 15, l - 32, null,
									2f, 2f);
						}
